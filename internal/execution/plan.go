...
func runPlan(ctx context.Context, config config.Config, secretsData []byte) []setup.Step {
	...
	if config.Env.CI {
		plan = append(
			plan,
			steps.ExistingRepo(config.Repo.SSH, config.Machine.RepoDir, revision.Commit),
		)
	} else {
		plan = append(
			plan,
			steps.CloneRepo(config.Repo.HTTP, config.Repo.SSH, config.Machine.RepoDir, revision.Commit),
		)
	}
